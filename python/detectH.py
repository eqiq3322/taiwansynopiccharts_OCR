...
DEFAULT_IMG = "../sfcmap"
DEFAULT_DATA = "../DATA"
DEFAULT_OUTPUT = "html/detectH.html"
DEFAULT_START = "2019-11-01 00:00"
DEFAULT_END = "2019-11-03 23:59"

parser = argparse.ArgumentParser(description="Detect high-pressure centers from synoptic maps.")
...
cfg = load_config_section(args.config, "detectH")
...
if __name__ == "__main__":
    print("=== Start Processing (Labels Inside Frame & English Comments) ===")
    ...
    if final_data:
        ...
        # Configure Geo Layout (Conic projection, bigger base map for labels)
        fig.update_geos(
            projection_type="conic conformal",
            projection_parallels=[30, 60],
            projection_rotation=dict(lon=120),
            
            # Base map range (must cover the labels on the frame edges)
            lataxis_range=[10, 60],
            lonaxis_range=[100, 150],
        ...
        # === Add Manual Labels using go.Scattergeo (INSIDE THE FRAME) ===
        
        # 1. Longitude Labels (Bottom edge at 10N)
        lon_labels = list(range(100, 151, 10)) # 100, 110... 150
        fig.add_trace(go.Scattergeo(
            mode="text",
            lon=lon_labels,
            lat=[10] * len(lon_labels), # Anchor on 10N line
            text=[f"{l}E" for l in lon_labels],
            # Changed from "bottom center" to "top center" to put inside frame
        ...
        lat_labels = list(range(10, 61, 10)) # 10, 20... 60
        fig.add_trace(go.Scattergeo(
            mode="text",
            lon=[100] * len(lat_labels), # Anchor on 100E line
            lat=lat_labels,
            text=[f"{l}N" for l in lat_labels],
            # Changed from "middle left" to "middle right" to put inside frame
        ...
        fig.add_trace(go.Scattergeo(
            mode="text",
            lon=[150] * len(lat_labels), # Anchor on 150E line
            lat=lat_labels,
            text=[f"{l}N" for l in lat_labels],
            # Changed from "middle right" to "middle left" to put inside frame
